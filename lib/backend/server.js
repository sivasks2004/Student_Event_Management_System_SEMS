...
app.post("/register-event", async (req, res) => {
  try {
    ...
    if (!name || !email || !eventName || !college || !contact || !rollNumber ||
        ...
        !certificationLink || !interOrIntraEvent) {
      return res.status(400).json({ message: "All fields are required" });
    }

    const newEvent = new Event({
      name, email, eventName, college, contact, rollNumber, symposiumName,
      eventType, teamOrIndividual, teamMembers, eventDate: new Date(eventDate), eventDaysSpent,
      prizeAmount, positionSecured, certificationLink, interOrIntraEvent
    });
    await newEvent.save();
...
app.get("/view-events", async (req, res) => {
  try {
    ...
    if (fromDate && toDate) {
      query.eventDate = { $gte: new Date(fromDate), $lte: new Date(toDate) };
    } else if (fromDate) {
